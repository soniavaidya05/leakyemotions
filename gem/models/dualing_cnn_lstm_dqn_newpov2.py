...
class CNN_CLD(nn.Module):
    def __init__(self, in_channels, num_filters, filter_size, stride):
    ...
    def forward(self, x):
        ...
        y1 = F.relu(self.conv_layer2(y1))
        y1 = F.relu(self.conv_layer3(y1))
...
class Combine_CLD(nn.Module):
    ...
    def __init__(
        ...
        self.cnn = CNN_CLD(in_channels, num_filters, filter_size, stride)
        self.rnn = nn.LSTM(
            input_size=in_size,
...
class Model_CNN_LSTM_DQN:
    ...
    def pov(self, world, location, holdObject, inventory=[], layers=[0]):
        ...
        for layer in layers:
            ...
            img = agent_visualfield(
                world,
                loc,
                tile_size=holdObject.appearance.shape[:2],
                k=holdObject.vision,
            )
    ...
    def training(self, batch_size, gamma):
        ...
        if current_replay_size > batch_size:
            ...
            state1_batch = torch.cat([s1 for (s1, a, r, s2, d) in minibatch]).to(
                self.device
            )
            ...
            state2_batch = torch.cat([s2 for (s1, a, r, s2, d) in minibatch]).to(
                self.device
            )

        ...
        while done == 0:
            ...
            for loc in agentList:
                if env.world[loc].kind != "deadAgent":
                    ...
                    # error = 1
                    gamma = 0.99
                    Q1 = models[policy].model1(state)
                    with torch.no_grad():
                        Q2 = models[policy].model2(next_state)

                    Y = reward + gamma * ((1 - done) * torch.max(Q2.detach(), dim=1)[0])

                    X = Q1.detach()[0][action]
                    # print(Q1.detach(), dim=1[action])
                    # print(action)
                    # X = Q1.gather(dim=1, index=action.long().unsqueeze(dim=1)).squeeze()
...
for modRun in range(len(run_params)):
    ...
    save_models(
        models,
        save_dir,
        "taxi_cab_basic_PER_atDec" + str(modRun),
    )
    make_video(
        "taxi_cab_basic_PER_atDec" + str(modRun),
        save_dir,

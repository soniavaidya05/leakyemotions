...
class Agent:
    ...
    def init_replay(self, numberMemories, pov_size=9, visual_depth=6):
        """
    ...
    def transition(self, env, models, action, location):
        ...
        if action in [0, 1, 2, 3]:
            ...
            if isinstance(env.world[attempted_locaton], House):
                reward = -0.1
                outcome = 3

            if isinstance(env.world[attempted_locaton], Wood):
                if self.wood_skill < random.random():
                    ...
                    self.wood += 1
                    env.world[attempted_locaton] = self
                    env.world[attempted_locaton] = EmptyObject()
                    env.world[location] = EmptyObject()
                    new_loc = attempted_locaton
                    outcome = 4
                else:
            ...
            if isinstance(env.world[attempted_locaton], Stone):
        ...
        if action == 4:
            ...
            if self.stone > 0 and self.wood > 0 and self.house_skill < random.random():
                posloc1 = self.movement(0, location)
                posloc2 = self.movement(1, location)
                posloc3 = self.movement(2, location)
                posloc4 = self.movement(3, location)
                posloc = []
                if isinstance(env.world[posloc1], EmptyObject):
                    posloc.append(0)
                if isinstance(env.world[posloc2], EmptyObject):
                    posloc.append(1)
                if isinstance(env.world[posloc3], EmptyObject):
                    posloc.append(2)
                if isinstance(env.world[posloc4], EmptyObject):
                    posloc.append(3)
                if len(posloc) > 0:
                    if isinstance(env.world[attempted_locaton], EmptyObject):
                        reward = 20
                        self.stone -= 1
                        self.wood -= 1
                        env.world[attempted_locaton] = House()
                        outcome = 8
                        jumpLocation = np.random.choice(posloc)
                        if jumpLocation == 0:
                            attempted_locaton = posloc1
                        if jumpLocation == 1:
                            attempted_locaton = posloc2
                        if jumpLocation == 2:
                            attempted_locaton = posloc3
                        if jumpLocation == 3:
                            attempted_locaton = posloc4
                    env.world[attempted_locaton] = self
                    new_loc = attempted_locaton
                    env.world[location] = EmptyObject()
        ...
        # print("location", location, "new location", new_loc, "action", action)

...
class Model_CNN_LSTM_AC:
    ...
    def __init__(
        self, in_channels, numFilters, lr, replay_size, in_size, hid_size1, hid_size2, out_size
    ):
        self.modeltype = "cnn_lstm_ac"
        self.model1 = Combine_CLD_AC(in_channels,
            numFilters, in_size, hid_size1, hid_size2, out_size
        )
        self.model2 = Combine_CLD_AC(in_channels,
            numFilters, in_size, hid_size1, hid_size2, out_size
        )
        ...
        self.device = "cpu" # need to update this
    ...
    def transfer_memories(self, world, loc):
        ...
        """
        # we may want everything to be tensors all the way
        
        rewards = world[loc].AC_reward.flip(dims=(0,)).view(-1)
        logprobs = world[loc].AC_logprobs.flip(dims=(0,)).view(-1)
        values = world[loc].AC_values.flip(dims=(0,)).view(-1)

        # this converts list to tensor

        #rewards = torch.tensor(world[loc].AC_reward).float().flip(dims=(0,)).view(-1)
        #logprobs = torch.tensor(world[loc].AC_logprobs).float().flip(dims=(0,)).view(-1)
        #values =torch.tensor(world[loc].AC_values).float().flip(dims=(0,)).view(-1)

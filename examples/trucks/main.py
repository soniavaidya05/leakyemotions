...
# From the trucks directory, we need to go up two folders
elif current_path[-1] == 'trucks':
    root = os.path.abspath('../..')
...
def run_game(
    ...
    if cfg.log:
        ...
        writer = SummaryWriter(
            log_dir=f'{root}/examples/trucks/runs/{datetime.now().strftime("%Y%m%d-%H%m%s")}/'
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', '-c', default=f'{root}/examples/trucks/configs/config.yaml', help='The filepath to YAML configuration file.')

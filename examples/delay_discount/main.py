...
from examples.delay_discount.iRainbow_clean import iRainbowModel
from examples.delay_discount.env import RPG
...
from examples.delay_discount.elements import EmptyObject, Wall
        ...
        while done == 0:
            ...
            for loc in agentList:
                if env.world[loc].kind != "deadAgent":
                    ...
                    #    [
                    #        state,
                    #        torch.tensor(local_gem_count[0]).float() * 100,
                    #        torch.tensor(local_gem_count[1]).float() * 100,
                    #    ]
                    # )
                    # print(state.shape)
                    ...
                    if reward == 5:
                        gems[0] = gems[0] + 1
                        local_gem_count[0] = local_gem_count[0] + 1
                    if reward == 2:
                        gems[1] = gems[1] + 1
                        local_gem_count[1] = local_gem_count[1] + 1
                    if reward == -5:
                        gems[2] = gems[2] + 1
                    if reward == -1:
                        gems[3] = gems[3] + 1
                    ...
                    # note - this needs to be thought about for location for multi-agent
                    if (

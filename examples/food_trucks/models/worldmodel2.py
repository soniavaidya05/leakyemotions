...
class EncoderDecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        # Current state
        x1 = self.decoder_layers(x)
        # Sigmoid transformation
        x1 = self.sigmoid(x1)

        # Next state
        x2 = self.decoder_layers(x)
        # Sigmoid transformation
        x2 = self.sigmoid(x2)

        return x1, x2, actions
...
class WorldModel(nn.Module):
    ...
    def __init__(
        ...
        self.model = self.build_model(linear=linear)
        self.optim = optim.Adam(self.model.parameters(), lr = .001)
    ...
    def build_linear_layers(self, reversed = False, linear = False):
        ...
        layer_config = [
            ...
            {'layer_type': 'relu'},
            {
                ...
                'out_features': self.cnn_config[0]['out_channels'] * self.cnn_config[0]['kernel_size'] * self.cnn_config[0]['stride']
            },
        ]
    ...
    def build_model(self, linear = False):
        '''
        ...
        action_layers = nn.Sequential(
            nn.Linear(
                in_features = self.cnn_config[0]['out_channels'] * self.cnn_config[0]['kernel_size'] * self.cnn_config[0]['stride'],
    ...
    def train(self):
        ...
        predicted_states, predicted_next_states, predicted_actions = self.model(states)
        ...
        return 0, r1_loss.item(), r2_loss.item()
...
def sample_and_visualize_test(model, 
    ...
    if action_model == None:
        ...
        for each_sample in sample:
            ...
            with torch.no_grad():
                predicted_state, predicted_next_state, _ = model.encoder(state_tensor)
        ...
        while done == 0:
            ...
            for loc in agentList:
                ...
                with torch.no_grad():
                    predicted_state, predicted_next_state, _ = model.model(state_tensor)

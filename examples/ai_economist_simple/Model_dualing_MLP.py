...
class MLP_DQN(nn.Module):
    ...
    def __init__(
        ...
        self.l2 = nn.Linear(hid_size1, hid_size1)
        self.l3 = nn.Linear(hid_size1, hid_size2)
    ...
    def forward(self, x):
        ...
        cnn_out = x.flatten(start_dim=1)

        y = F.relu(self.l1(cnn_out))
        y = F.relu(self.l2(y))
        y = F.relu(self.l3(y))
...
class Model_linear_MLP_DDQN:

    kind = "linear_mlp_ddqn"  # class variable shared by all instances
    ...
    def __init__(
        ...
        self.modeltype = "linear_lstm_mlp"
        self.model1 = MLP_DQN(
